...
<div class="custom-bg">
    <h3 class="text-center mb-4 text-white">Data Keuntungan Penjualan</h3>
            ...
            <table id="transaksiTable" class="table table-striped table-bordered">
                ...
                @foreach ($transaksis as $transaksi)
                    @foreach ($transaksi['details'] as $detail)
                        @php
                            $hargaJual = $produkMap[$detail['produk_id']]['harga_jual'];
                            $hargaBeli = $produkMap[$detail['produk_id']]['harga_beli'];
                            $quantity = $detail['quantity'];
                            $keuntungan = ($hargaJual - $hargaBeli) * $quantity;
                        @endphp
                        <tr>
                            <td>{{ $rowNumber++ }}</td>
                            ...
                            <td>{{ $produkMap[$detail['produk_id']]['stok'] }}</td>
                        </tr>
                    @endforeach
                @endforeach
                </tbody>
