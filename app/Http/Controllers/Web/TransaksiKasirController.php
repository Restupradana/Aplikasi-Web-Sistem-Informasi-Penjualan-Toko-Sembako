...
class TransaksiKasirController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($response->getStatusCode() == 200 && $data['status'] == true) {
            return view('transaksi_kasir.index', ['transaksis' => $data['data']]);
        } else {
            return view('transaksi_kasir.index', ['transaksis' => [], 'error' => 'Failed to fetch data']);
        }
    }
    ...
    public function create()
    {
        ...
        return view('transaksi_kasir.create', [
    ...
    public function store(Request $request)
    {
        ...
        if ($data['status']) {
            return redirect()->route('transaksi_kasir.index')->with('success', 'Transaction created successfully.');
        }
